function heightmapTerrainAlgorithm() {
    ...
    this.triPlanerMap =  false;
    this.detailDistance = 50;
    this.detailDistanceFlat = 500;
    ...
    this.poolInit = function(cb, params) {
    	...
    	this.normalMapping = window._SettingsManager.getKey('normalMapTerrain') && !window._SettingsManager.getKey('useSimpleMaterials') && window._dRenderer.supportsStandardDerivatives();
    	this.triPlanerMap =  window._SettingsManager.getKey('triPlanerMapTerrain') ;
    ...
    this.getMaterialUniforms = function(mesh, matrix) {
        ...
        var uniforms_default = {
            diffuseSampler: {
                type: "t",
                value: this.getTexture(this.diffuseUrl, true)
            },
            baseSampler: {
                type: "t",
                value: this.getTexture(this.baseUrl || "terrain/ground.jpg", true)
            },
            gSampler: {
                type: "t",
                value: this.getTexture(this.gUrl || "terrain/cliff.jpg", true)
            },
            rSampler: {
                type: "t",
                value: this.getTexture(this.rUrl || "terrain/cliff.jpg", true)
            },
            bSampler: {
                type: "t",
                value: this.getTexture(this.bUrl || "terrain/ground.jpg", true)
            },

            baseNormalMap: {
                type: "t",
                value: this.getTexture(this.baseUrlNorm || "terrain/3091-normal.jpg", true)
            },
            gNormalMap: {
                type: "t",
                value: this.getTexture(this.gUrlNorm || "terrain/grassnorm.jpg", true)
            },
            rNormalMap: {
                type: "t",
                value: this.getTexture(this.rUrlNorm || "terrain/4979-normal.jpg", true)
            },
            bNormalMap: {
                type: "t",
                value: this.getTexture(this.bUrlNorm || "textures/waternormal.jpg", true)
            },

            mixMap: {
                type: "t",
                value: this.getTexture(this.mixUrl || "terrain/rivermix.png", true)
            },
    ...
    this.getNormalFragmentShader = function() {
        ...
        return "" +
            ...
            "	return (blend_weights.x * dirt0 + blend_weights.y * dirt1 + blend_weights.z * dirt2) / (blend_weights.x + blend_weights.y + blend_weights.z);\n" +
            "}" +
            ...
            (this.normalMapping ?
                ...
                "	 lowp vec3 basemap = perturb_normal(TBN2,coordsScaleA.yx,baseNormalMap).rgb;\n" +
                "	 lowp vec3 rmap = perturb_normal(TBN2,coordsScaleA.yx,rNormalMap).rgb;\n" +
                "	 lowp vec3 gmap = perturb_normal(TBN2,coordsScaleA.yx,gNormalMap).rgb;\n" +
                "	 lowp vec3 bmap = perturb_normal(TBN2,coordsScaleA.yx,bNormalMap).rgb;\n"
                ...
                "	 lowp vec3 basenear = perturb_normal(TBN2,coordsScaleB.yx,baseNormalMap).rgb* coordA/coordB;\n" +
                "	 lowp vec3 rnear = perturb_normal(TBN2,coordsScaleB.yx,rNormalMap).rgb* coordA/coordB;\n" +
                "	 lowp vec3 gnear = perturb_normal(TBN2,coordsScaleB.yx,gNormalMap).rgb* coordA/coordB;\n" +
                "	 lowp vec3 bnear = perturb_normal(TBN2,coordsScaleB.yx,bNormalMap).rgb* coordA/coordB;\n"
                	)
                ) +
    ...
    this.getDiffuseFragmentShader = function(mesh, matrix) {
        ...
        return (
            ...
            (
            	...
            	"	lowp vec4 basemap = texture2D(baseSampler,coordsScaleA.yx);\n" +
            	"	lowp vec4 rmap = texture2D(rSampler,coordsScaleA.yx);\n" +
            	"	lowp vec4 gmap = texture2D(gSampler,coordsScaleA.yx);\n" +
            	"	lowp vec4 bmap = texture2D(bSampler,coordsScaleA.yx);\n"
            ...
            "		lowp vec4 basenear = texture2D(baseSampler,coordsScaleB.yx);\n" +
            	"	lowp vec4 rnear = texture2D(rSampler,coordsScaleB.yx);\n" +
            	"	lowp vec4 gnear = texture2D(gSampler,coordsScaleB.yx);\n" +
            	"	lowp vec4 bnear = texture2D(bSampler,coordsScaleB.yx);\n"
            ))+
            ...
            "	return mix(near,medAndFar,nearamt)*1.5;\n" +  //the 1.5 here is to adjust, becasue an old error when blending made things too bright. This makes it look closer to how it use too, but is technicall wrong
            "}")
    }
    ...
    this.sampleBiCubic = function(u, v, matrix, tileres) {
        ...
        if ((this.worldWidth / this.dataWidth) < 2)
            res = Math.min(1, (this.worldWidth / this.dataWidth) / 50); //need to review this.... intended to smooth when heightmap values are too high frequency, might do more harm than good
        var mip = 1 / res;
