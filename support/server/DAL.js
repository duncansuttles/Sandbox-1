...
function getInventoryItemAssetData(userID, inventoryID, cb)
{
    getInventoryForUser(userID, function(inventory, Ikey)
    {
        ...
        if (inventory.indexOf(inventoryID) != -1)
        {
            fs.readFile((datapath + '/Profiles/' + userID + '_Data' + '/' + inventoryID)
            ...
            function()
            {
                ...
                DB.remove(idk, function(err, doc, key)
                {
                    ...
                    DB.listDepend('UserIndex', id, function(err, UserIndex, key)
                    {
                        ...
                        {
                            //delete user folder
                            deleteFolderRecursive((datapath + '/Profiles/' + id)
                                .replace(safePathRE) + '_Data');
                            cb();
                        });
                    });
                });
    ...
    getInstance(id, function(instance)
    {
        if (instance)
        {
            async.waterfall([
                function(cb2)
                {
                    MakeDirIfNotExist((datapath + '/States/' + id)
                        .replace(safePathRE),
                        function()
                        {
                            cb2();
                ...
                function(cb2)
                {
                    CheckHash((datapath + '/States/' + id + '/state')
                        .replace(safePathRE), parsedData,
                        function(issame)
                        {
                ...
                function(issame, cb2)
                {
                    if (!issame)
                    {
                        SaveFile((datapath + '/States/' + id + '/state')
                            .replace(safePathRE), JSON.stringify(parsedData),
                            function()
                            {
                                cb2(undefined);
                            });
...
{
    var profilePath = (datapath + '/Profiles/')
        .replace(safePathRE);
            ...
            async.eachSeries(files,
                function(i, cb)
                {
                    ...
                    getInstance(i, function(inst)
                    {
                        ...
                        {
                            var stateexists = fs.existsSync((datapath + "/states/" + i + "/state")
                                .replace(safePathRE), 'utf8');
                            if (stateexists)
                            {
                                var instdata = fs.readFileSync((datapath + "/states/" + i + "/state")
                                    .replace(safePathRE), 'utf8');
